...
# @date 2025-09-21
# @version 1.0
# @brief Ejemplo de control bajo nivel de G1 de 23 DoF en simulación
#
# @descripcion
#   Este script demuestra cómo enviar comandos LowCmd al robot G1 de 23 DoF
#   dentro del simulador Unitree Mujoco. Ejecuta una secuencia de movimiento
#   de tobillos y muñecas del robot.
#
# @requisitos
#   - Unitree mujoco instalado y configurado.
#   - SDK2 de Unitree (C++/Python) instalado en el sistema.
#   - Simulador G1 de 23 DoF corriendo con `enable_elastic_band = 1`.
#
# @uso
...
class Custom:
        ...
        if self.time_ < self.duration_:
            ...
            for i in range(G1_NUM_MOTOR):
                ...
                self.low_cmd.motor_cmd[i].q = (
                    (1.0 - ratio) * self.low_state.motor_state[i].q
                )
    ...
    def StopAndShutdown(self, repeat: int = 50, delay: float = 0.05):
        """Detiene hilo de control y pone todos los motores a cero de forma segura."""
        if hasattr(self, 'lowCmdWriteThreadPtr'):
            self.lowCmdWriteThreadPtr.Wait()
            print("[INFO] Control thread stopped.")

        if self.low_state is None:
            print("[WARNING] LowState not received yet. Cannot reset motors safely.")
            return

        print("[INFO] Resetting motors to zero...")
        zero_cmd = unitree_hg_msg_dds__LowCmd_()
        for i in range(G1_NUM_MOTOR):
            zero_cmd.mode_pr = Mode.PR
            zero_cmd.mode_machine = self.mode_machine_
            zero_cmd.motor_cmd[i].mode = 1
            zero_cmd.motor_cmd[i].q = 0.0
            zero_cmd.motor_cmd[i].dq = 0.0
            zero_cmd.motor_cmd[i].tau = 0.0
            zero_cmd.motor_cmd[i].kp = Kp[i]
            zero_cmd.motor_cmd[i].kd = Kd[i]

        zero_cmd.crc = self.crc.Crc(zero_cmd)

        for _ in range(repeat):
            self.lowcmd_publisher_.Write(zero_cmd)
            time.sleep(delay)

        print("[INFO] Motors successfully set to zero.")
...
if __name__ == '__main__':
    ...
    try:
        ...
        custom.StopAndShutdown()  # Detiene hilo y pone motores a cero
        print("[INFO] Program terminated safely.")
